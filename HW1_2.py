...
def generate_data(dim, data_size):
	mean_1 = [0]; mean_1.extend(zeros(dim-1)) 
	cov_1 = eye(dim, dtype=int)
...
def main():
	...
	list_dim = range(1, 110, 10)
	# list_dim = [1, 11]
	for dim in list_dim:
		train_x, train_y = generate_data(dim, train_data_size)
		test_x, test_y = generate_data(dim, test_data_size)

		# NOTE: bug in custom code so ended up using scikit

		clf = KNeighborsClassifier(1)
		clf.fit(train_x, train_y)
		pred_y = clf.predict(test_x)
		# list_error_1NN.appennd(sum (1 for i in range(len(pred_y) if pred_y[i]!=test_y[i]))
		list_error_1NN.append(accuracy_score(test_y, pred_y))
		
		clf = KNeighborsClassifier(3)
		clf.fit(train_x, train_y)
		pred_y = clf.predict(test_x)		
		# list_error_3NN.appennd(sum (1 for i in range(len(pred_y) if pred_y[i]!=test_y[i]))
		list_error_3NN.append(accuracy_score(test_y, pred_y))
