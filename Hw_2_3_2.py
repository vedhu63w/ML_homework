...
def Laplace_kernel(X, s):
	pairwise_dists = squareform(pdist(X, 'euclidean'))
	return scipy.exp(-pairwise_dists / s)

def laplace_kernel(x1, x2, s):
    return exp(-(norm(x1 - x2)) / s)
...
def generate_kernel_matrix(X, s):
    ...
    for i in range(n):
        for j in range(n):
            xi, xj = X[i], X[j]
            matrix[i,j] = laplace_kernel(xi, xj, s)
    return matrix
	...
	for lambd in list_lambd:
		...
		for i in range(len(test_data)):
			...
			for j in range(len(data['d79'])):
				norm_sq = norm(test_data[i] - train_data[j])	
				# print norm_sq
				a_j += alpha[j]*scipy.exp(-norm_sq / s)
